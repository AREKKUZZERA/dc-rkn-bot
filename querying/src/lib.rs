...
impl Checker {
    ...
    pub async fn blocked_domains(&self) -> Vec<String> {
        self.ru_blacklist.load().all_domains().to_vec()
    }

    pub async fn blocked_subnets(&self) -> Vec<IpNet> {
        self.ru_blacklist.load().all_subnets().to_vec()
    }

    pub async fn total_blocked_entries(&self) -> usize {
        let ru_blacklist = self.ru_blacklist.load();
        ru_blacklist.domain_count + ru_blacklist.all_subnets().len()
    }
