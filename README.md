...
## API для списка блокировок

В `website` добавлены JSON endpoints для работы с полным списком блокировок и дельтой между обновлениями:

- `GET /api/blocked?page=1&limit=50` — пагинированный список
- `GET /api/blocked?query=discord&kind=domain` — поиск
- `GET /api/updates` — новые записи с последнего обновления баз
- `GET /api/stats` — статистика

`/api/updates` возвращает только те записи, которые появились при последнем refresh источников. Первый успешный refresh используется как базовый snapshot и не считается «новыми блокировками».

## Discord бот

В папке `discord-bot/` лежит готовый бот, который работает поверх этих endpoints.
...
Бот также может слать уведомления о новых блокировках в канал Discord по `DISCORD_NOTIFY_CHANNEL_ID`.
